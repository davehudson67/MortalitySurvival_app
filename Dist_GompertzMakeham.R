...
rgompzMake <- nimbleFunction(
  run = function(n = integer(0), a = double(0),
    ...
    if(n != 1) print("rgompzMake only allows n = 1; using n = 1.")
    u <- runif(1, 0, 1)
...
registerDistributions(list(
  dgompzMake = list(
    BUGSdist = "dgompzMake(a, b, c)",
    Rdist = "dgompzMake(a, b, c)",
    pqAvail = TRUE, 
    range = c(0, Inf)
  )
))
